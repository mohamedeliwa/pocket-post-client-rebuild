...
const Author = (props) => {
  ...
  const { data: allPosts, error } = useSWR(`https://pocket-post-server.herokuapp.com/posts?user_id=${props.user._id}&limit=4&skip=0&sortBy=createdAt:desc`, fetcher);
...
export async function getServerSideProps(context) {
  ...
  const res = await fetch(`https://pocket-post-server.herokuapp.com/users/profile/${id}`);
  const user = await res.json();
  user.avatar = `https://pocket-post-server.herokuapp.com/users/${id}/avatar`
  // fechting user's post collections
  const res2 = await fetch(`https://pocket-post-server.herokuapp.com/series?user_id=${id}&sortBy=createdAt:desc`);
